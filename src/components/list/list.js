...
class List extends Component {
	...
	componentDidMount() {
		...
		this.populateList(this.state.items, itemsList);
	}
	...
	handleForm = (e) => {
		...
		items.push(item);
		//WORKS OPT:
		//finds 'value' prop based on the object structure of the event calling the function.
		// let item = e.target.children[0].value;

		//BETTER OPT:
		//'this'is the object that calls the function. In this instance it is <form class="add-items"> when a 'submit' event is triggered.
		//You can then use 'this' to find the 'value' prop based on <input> 'name' prop.
		//This makes it more futureproof as future changes to the structure of the form will not effect finding the value.
	...
	populateList = (items = [], itemList) => {
		itemList.innerHTML = items
			.map((item, i) => {
				...
				//WORKS OPT:
				//creating and appending child nodes (and removing inappropriate ones) can work but will require specific naming to identify children to remove.
				// replacing the entire innerHTML allows for simplier implementations - this may have performance costs though as all elements are  being build every submit.
				...
				//BETTER OPT:
				//innerHTML is the html elements rendered to the browser in string form.
				//This can be changed to render any HTML elements using a string.
				//This will change all the children nodes.
